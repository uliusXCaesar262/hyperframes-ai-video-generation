...
def lower_third(name, species, accent, x=48, y=1486, w=900):
    """Animated lower-third: slides in from the left over 0.5s (t=0.25..0.75)."""
    h = 150
    E = "if(lt(t,0.25),-968,if(lt(t,0.75),-968+(t-0.25)*1936,0))"  # x-offset: -968 -> 0
    return (
        f"drawbox=x='{x}+({E})':y={y}:w={w}:h={h}:color=black@0.5:t=fill,"
        f"drawbox=x='{x}+({E})':y={y}:w=12:h={h}:color={accent}:t=fill,"
        f"drawtext=fontfile={FB}:text='{name}':fontcolor=white:fontsize=62:x='{x+44}+({E})':y={y+22}:shadowcolor=black@0.6:shadowx=2:shadowy=2,"
        f"drawtext=fontfile={FR}:text='{species}':fontcolor={DIM}:fontsize=34:x='{x+46}+({E})':y={y+96}"
    )
...
D0 = 2.8
vf0 = (
    "scale=1080:1920,setsar=1,fps=30,fade=t=in:st=0:d=0.45,eq=brightness=-0.03,"
    f"drawtext=fontfile={FB}:text='IL SOLCO GENGIVALE':fontcolor=white:fontsize=66:x=(w-text_w)/2:y=150:box=1:boxcolor=black@0.45:boxborderw=22:alpha='if(lt(t,0.5),0,if(lt(t,1.0),(t-0.5)/0.5,1))',"
    f"drawtext=fontfile={FR}:text='campo di battaglia invisibile':fontcolor={GOLD}:fontsize=40:x=(w-text_w)/2:y=252:alpha='if(lt(t,0.7),0,if(lt(t,1.2),(t-0.7)/0.5,1))',"
    "vignette=PI/5"
)
...
D1 = 8.0
vf1 = "scale=1080:1920,setsar=1,fps=30," + lower_third("PORPHY","Porphyromonas gingivalis",RED) + ",vignette=PI/5"
...
vf2 = (
    ...
    f"{lower_third('PREVI','Prevotella',PURP)},vignette=PI/5[v]"
)
...
vf3 = (
    ...
    f"{lower_third('STREP. DENTISANI','il batterio buono',GOLD)},vignette=PI/5[v]"
)
...
D4 = 3.4
vf4 = (
    "scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,gblur=sigma=30,eq=brightness=-0.28:saturation=0.9,setsar=1,fps=30,fade=t=in:st=0:d=0.4,"
...
run(["ffmpeg","-y",*inputs,"-filter_complex",f"{pre}{maps}concat=n={n}:v=1:a=1[v][a]",
     "-map","[v]","-map","[a]",*VENC,*AENC,"-movflags","+faststart",f"{OUT}/microbioma-orale-v1_5.mp4"])

print("\nDONE ->", f"{OUT}/microbioma-orale-v1_5.mp4")
